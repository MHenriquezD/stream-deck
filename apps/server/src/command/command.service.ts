...
export class CommandService {
  ...
  async execute(id: string) {
    ...
    const verdict = validateCommand(
      command.payload,
      process.platform,
      this.getTrustedPayloads(),
    );
    if (!verdict.allowed) {
      throw new ForbiddenException(verdict.reason ?? 'Command not allowed');
    }
  ...
  /** Payloads generated by the server itself, trusted verbatim by the validator. */
  private getTrustedPayloads(): Set<string> {
    return new Set(this.getMultimediaPresets().map((p) => p.payload));
  }
